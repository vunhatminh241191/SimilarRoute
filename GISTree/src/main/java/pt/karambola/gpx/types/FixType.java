...
package pt.karambola.gpx.types;
/**
 * 
...
FixType
{
	...
	{
		if (NONE.getValue().equals(value))    return NONE ;
		if (TWO_D.getValue().equals(value))   return TWO_D;
		if (THREE_D.getValue().equals(value)) return THREE_D;
		if (DGPS.getValue().equals(value))	  return DGPS;
		if (PPS.getValue().equals(value)) 	  return PPS;
